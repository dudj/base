...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/comments/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	function get_html(url,title){
		$fly.gethtml(url,title);
	}
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/comments/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	...
	function del(id){
		...
		if (r){
			$.get('/admin/comments/del',{id:id},function(data){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			striped: true,//显示行条纹
			url: '/admin/comments/list_page',
			queryParams:{user_infoid:$fly.request('user_infoid'),shop_infoid:$fly.request('shop_infoid'),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},
			remoteSort: false,
			//idField: 'id',
			...
			columns: [[
			   ...
			   { field: 'content', title: '评论内容', align: 'center', width: 100,sortable:true },
			   { field: 'commentsimg', title: '评论图片', align: 'center', width: 100,sortable:true },
			   { field: 'grade', title: '评论级别', align: 'center', width: 100,sortable:true },
			   { field: 'nick_name', title: '用户昵称', align: 'center', width: 100,sortable:true },
			   { field: 'shop_name', title: '商家名称', align: 'center', width: 100,sortable:true },
			...
			toolbar: [{text: '删 除',iconCls: 'icon-cancel',handler: function () {var idlist = $fly.getchecked();if(idlist==''){$fly.msg('您没有勾选任何数据！');return false;}$.messager.confirm('友情提示','确定要删除选中吗？',function(r){if (r){var idlist = $fly.getchecked();$.post('/admin/comments/delall',{idlist:idlist},function(){$('#mytable').datagrid('reload');});}});}}, '-',{text: '导 出',iconCls: 'icon-redo',handler: function () {
                var win = $.messager.progress({
                    title:'请稍等',
                    msg:'正在导出...'
                });
                $.post('/admin/comments/list_page',{daochu:1,user_infoid:$fly.request('user_infoid'),shop_infoid:$fly.request('shop_infoid'),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},function(data){
                    data = eval('['+data+']');
                    data = data[0];
                    $.messager.progress('close');
                    if(data.result){
                        window.location.href='/comments.xls';
                    }
