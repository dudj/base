...
<head id="Head1">
	...
	function del(id){
		...
		if (r){
			$.get('/admin/admin/del',{id:id},function(){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			columns: [[
			   ...
			   { field: 'name', title: '商户名', align: 'center',width: 220 },
			   { field: 'cityname', title: '所属城区', align: 'center',width: 220 },
			...
			toolbar: [{
				...
				iconCls: 'icon-add',
				handler: function () {
					$fly.gethtml('/admin/admin/add','添加商户');
				}
			}, '-', {
				text: '删 除',
				iconCls: 'icon-cancel',
				handler: function () {
					...
					if (r){
						var idlist = $fly.getchecked();
						$.post('/admin/admin/delall',{idlist:idlist},function(){
							$('#mytable').datagrid('reload');
						});
					}
					});
