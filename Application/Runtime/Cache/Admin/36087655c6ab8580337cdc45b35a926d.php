...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/news/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	...
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/news/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			striped: true,//显示行条纹
			url: '/admin/news/list_page',
			queryParams:{news_type:$('#news_type').val(),user_type:$('#user_type').val(),userid:$fly.request('userid'),sendxqid:$fly.request('sendxqid'),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},
			remoteSort: false,
			//idField: 'id',
			...
			columns: [[
			   ...
			   { field: 'news_phone', title: '用户手机号', align: 'center', width: 100,sortable:true },{ field: 'news_content', title: '消息内容', align: 'center', width: 100,sortable:true },{ field: 'news_type', title: '消息类型', align: 'center', width: 100,sortable:true },{ field: 'user_type', title: '用户分类', align: 'center', width: 100,sortable:true },
...
<form id="sea" name="sea">
		<span>消息类型：</span><?php echo $c109;?><input type="hidden"> &nbsp;<span>用户分类：</span><?php echo $c77;?><input type="hidden"> &nbsp; <span>创建时间大于</span><input name="starttime" type="text"  onclick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" id="starttime" size="20" value="" class="easyui-validatebox" > 小于 <input name="endtime" type="text"  onclick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" id="endtime" value=""  size="20" class="easyui-validatebox"> 
