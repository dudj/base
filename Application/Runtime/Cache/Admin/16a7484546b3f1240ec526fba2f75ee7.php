...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/face/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	...
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/face/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	
	
	
	function image_(val,row){return '<img src="'+row.image+'" width=100>';}
	
	
	function del(id){
		$.messager.confirm('友情提示','确定要删除吗？',function(r){
		if (r){
			$.get('/admin/face/del',{id:id},function(data){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			striped: true,//显示行条纹
			url: '/admin/face/list_page',
			queryParams:{title:$('#title').val(),link:$('#link').val(),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},
			remoteSort: false,
			//idField: 'id',
			...
			columns: [[
			   ...
			   { field: 'title', title: '标题', align: 'center', width: 100,sortable:true },{ field: 'link', title: '链接', align: 'center', width: 100,sortable:true },{ field: 'image', title: '接口位置', formatter: image_,align: 'center', width: 100,sortable:true },
...
<form id="sea" name="sea">
		<span>标题：</span><input id="title" class="easyui-validatebox" size="10"> &nbsp;<span>链接：</span><input id="link" class="easyui-validatebox" size="10"> &nbsp; 
