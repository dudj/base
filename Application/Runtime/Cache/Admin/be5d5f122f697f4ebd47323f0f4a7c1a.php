...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/wechatkeywords/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	...
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/wechatkeywords/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	...
	function del(id){
		...
		if (r){
			$.get('/admin/wechatkeywords/del',{id:id},function(data){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			striped: true,//显示行条纹
			url: '/admin/wechatkeywords/list_page',
			queryParams:{type:$('#type').val(),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},
			remoteSort: false,
			//idField: 'id',
			...
			columns: [[
			   ...
			   { field: 'keyword', title: '关键字', align: 'center', width: 100,sortable:true },{ field: 'response', title: '回复内容', align: 'center', width: 100,sortable:true },{ field: 'type', title: '微信回复语类型', align: 'center', width: 100,sortable:true },
			...
			toolbar: [{text: '添 加',iconCls: 'icon-add',handler: function () {$fly.gethtml('/admin/wechatkeywords/add','添加');}}, '-',{text: '删 除',iconCls: 'icon-cancel',handler: function () {var idlist = $fly.getchecked();if(idlist==''){$fly.msg('您没有勾选任何数据！');return false;}$.messager.confirm('友情提示','确定要删除选中吗？',function(r){if (r){var idlist = $fly.getchecked();$.post('/admin/wechatkeywords/delall',{idlist:idlist},function(){$('#mytable').datagrid('reload');});}});}}, '-',{text: '导 出',iconCls: 'icon-redo',handler: function () {
                var win = $.messager.progress({
                    title:'请稍等',
                    msg:'正在导出...'
                });
                $.post('/admin/wechatkeywords/list_page',{daochu:1,type:$('#type').val(),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},function(data){
                    data = eval('['+data+']');
                    data = data[0];
                    $.messager.progress('close');
                    if(data.result){
                        window.location.href='/wechatkeywords.xls';
                    }
...
<form id="sea" name="sea">
		<span>微信回复语类型：</span><?php echo $c2;?><input type="hidden"> &nbsp; <span>创建时间大于</span><input name="starttime" type="text"  onclick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" id="starttime" size="20" value="" class="easyui-validatebox" > 小于 <input name="endtime" type="text"  onclick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" id="endtime" value=""  size="20" class="easyui-validatebox"> 
