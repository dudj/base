...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/sendxq/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	...
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/sendxq/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	...
	function del(id){
		...
		if (r){
			$.get('/admin/sendxq/del',{id:id},function(data){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			striped: true,//显示行条纹
			url: '/admin/sendxq/list_page',
			queryParams:{states:$('#states').val(),userid:$fly.request('userid'),gztypeid:$fly.request('gztypeid'),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},
			remoteSort: false,
			//idField: 'id',
			...
			columns: [[
			   ...
			   { field: 'address', title: '工程地点', align: 'center', width: 100,sortable:true },{ field: 'txt', title: '工作内容', align: 'center', width: 100,sortable:true },{ field: 'num', title: '需求人数', align: 'center', width: 100,sortable:true },{ field: 'startDate', title: '开始日期', align: 'center', width: 100,sortable:true },{ field: 'endDate', title: '竣工日期', align: 'center', width: 100,sortable:true },{ field: 'money', title: '价格区间', align: 'center', width: 100,sortable:true },{ field: 'isbx', title: '是否需要保险', align: 'center', width: 100,sortable:true },{ field: 'bf', title: '保险额度', align: 'center', width: 100,sortable:true },{ field: 'name', title: '联系人', align: 'center', width: 100,sortable:true },{ field: 'tel', title: '电话', align: 'center', width: 100,sortable:true },{ field: 'txt1', title: '描述', align: 'center', width: 100,sortable:true },{ field: 'ddh', title: '订单号', align: 'center', width: 100,sortable:true },{ field: 'lng', title: '经度', align: 'center', width: 100,sortable:true },{ field: 'lat', title: '维度', align: 'center', width: 100,sortable:true },{ field: 'states', title: '订单状态', align: 'center', width: 100,sortable:true },{ field: 'in_num', title: '现有人数', align: 'center', width: 100,sortable:true },
			...
			toolbar: [{text: '添 加',iconCls: 'icon-add',handler: function () {$fly.gethtml('/admin/sendxq/add','添加');}}, '-',{text: '删 除',iconCls: 'icon-cancel',handler: function () {var idlist = $fly.getchecked();if(idlist==''){$fly.msg('您没有勾选任何数据！');return false;}$.messager.confirm('友情提示','确定要删除选中吗？',function(r){if (r){var idlist = $fly.getchecked();$.post('/admin/sendxq/delall',{idlist:idlist},function(){$('#mytable').datagrid('reload');});}});}}]
		});
		//设置分页控件
...
<form id="sea" name="sea">
		<span>订单状态：</span><?php echo $c83;?><input type="hidden"> &nbsp; 
