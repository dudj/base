...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/orderlist/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	...
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/orderlist/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	...
	function del(id){
		...
		if (r){
			$.get('/admin/orderlist/del',{id:id},function(data){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			striped: true,//显示行条纹
			url: '/admin/orderlist/list_page',
			queryParams:{userid:$fly.request('userid'),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},
			remoteSort: false,
			//idField: 'id',
			...
			toolbar: [{text: '添 加',iconCls: 'icon-add',handler: function () {$fly.gethtml('/admin/orderlist/add','添加');}}, '-',{text: '删 除',iconCls: 'icon-cancel',handler: function () {var idlist = $fly.getchecked();if(idlist==''){$fly.msg('您没有勾选任何数据！');return false;}$.messager.confirm('友情提示','确定要删除选中吗？',function(r){if (r){var idlist = $fly.getchecked();$.post('/admin/orderlist/delall',{idlist:idlist},function(){$('#mytable').datagrid('reload');});}});}}]
		});
		//设置分页控件
