...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/orders/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	...
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/orders/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	...
	function del(id){
		...
		if (r){
			$.get('/admin/orders/del',{id:id},function(data){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			striped: true,//显示行条纹
			url: '/admin/orders/list_page',
			queryParams:{pay_way:$('#pay_way').val(),status:$('#status').val(),user_infoid:$fly.request('user_infoid'),shop_infoid:$fly.request('shop_infoid'),user_picsid:$fly.request('user_picsid'),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},
			remoteSort: false,
			//idField: 'id',
			...
			columns: [[
			   ...
			   { field: 'orderNumber', title: '订单号', align: 'center', width: 100,sortable:true },{ field: 'pay_way', title: '支付方式', align: 'center', width: 100,sortable:true },{ field: 'status', title: '订单状态', align: 'center', width: 100,sortable:true },{ field: 'money', title: '支付金额', align: 'center', width: 100,sortable:true },{ field: 'user_name', title: '收货人', align: 'center', width: 100,sortable:true },{ field: 'user_address', title: '收货地址', align: 'center', width: 100,sortable:true },{ field: 'tele_phone', title: '联系方式', align: 'center', width: 100,sortable:true },
			...
			toolbar: [{text: '添 加',iconCls: 'icon-add',handler: function () {$fly.gethtml('/admin/orders/add','添加');}}, '-',{text: '删 除',iconCls: 'icon-cancel',handler: function () {var idlist = $fly.getchecked();if(idlist==''){$fly.msg('您没有勾选任何数据！');return false;}$.messager.confirm('友情提示','确定要删除选中吗？',function(r){if (r){var idlist = $fly.getchecked();$.post('/admin/orders/delall',{idlist:idlist},function(){$('#mytable').datagrid('reload');});}});}}]
		});
		//设置分页控件
...
<form id="sea" name="sea">
		<span>支付方式：</span><?php echo $c2;?><input type="hidden"> &nbsp;<span>订单状态：</span><?php echo $c6;?><input type="hidden"> &nbsp; <span>创建时间大于</span><input name="starttime" type="text"  onclick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" id="starttime" size="20" value="" class="easyui-validatebox" > 小于 <input name="endtime" type="text"  onclick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" id="endtime" value=""  size="20" class="easyui-validatebox"> 
