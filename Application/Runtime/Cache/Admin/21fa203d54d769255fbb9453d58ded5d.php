...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/api2/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	...
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/api2/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	...
	function del(id){
		...
		if (r){
			$.get('/admin/api2/del',{id:id},function(data){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			columns: [[
			   ...
			   { field: 'name', title: '接口名称', align: 'center', width: 100,sortable:true },{ field: 'sql', title: 'sql语句', align: 'center', width: 100,sortable:true },{ field: 'par', title: '参数说明', align: 'center', width: 100,sortable:true },{ field: 'image', title: '图片', formatter: image_,align: 'center', width: 100,sortable:true },
			...
			toolbar: [{text: '添 加',iconCls: 'icon-add',handler: function () {$fly.gethtml('/admin/api2/add','添加');}}, '-',{text: '删 除',iconCls: 'icon-cancel',handler: function () {var idlist = $fly.getchecked();if(idlist==''){$fly.msg('您没有勾选任何数据！');return false;}$.messager.confirm('友情提示','确定要删除选中吗？',function(r){if (r){var idlist = $fly.getchecked();$.post('/admin/api2/delall',{idlist:idlist},function(){$('#mytable').datagrid('reload');});}});}}]
		});
		//设置分页控件
