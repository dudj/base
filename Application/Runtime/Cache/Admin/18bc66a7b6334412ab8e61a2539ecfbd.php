...
<head id="Head1">
	...
	function xiugai(val,row){
		return '<a href="javascript:get_html(\'/admin/order/update?id='+row.id+'\',\'修改\');"><font color="red">修改</font></a>';
	}
	...
	function xiangqing(val,row){
		return '<a href="javascript:get_html(\'/admin/order/xiangqing?id='+row.id+'\',\'详情\')"><font color="red">详情</font></a>';
	}
	...
	function del(id){
		...
		if (r){
			$.get('/admin/order/del',{id:id},function(data){
	...
	function loaddata(){
		...
		$('#mytable').datagrid({
			...
			striped: true,//显示行条纹
			url: '/admin/order/list_page',
			queryParams:{status:$('#status').val(),userid:$fly.request('userid'),sendxqid:$fly.request('sendxqid'),starttime:$("input[name='starttime']").val(),endtime:$("input[name='endtime']").val()},
			remoteSort: false,
			//idField: 'id',
			...
			columns: [[
			   ...
			   { field: 'nickname', title: '用户名称', align: 'center', width: 100,sortable:true },{ field: 'ddh', title: '订单号', align: 'center', width: 100,sortable:true },{ field: 'status', title: '工人状态', align: 'center', width: 100,sortable:true },
...
<form id="sea" name="sea">
		<span>工人状态：</span><?php echo $c88;?><input type="hidden"> &nbsp; <span>创建时间大于</span><input name="starttime" type="text"  onclick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" id="starttime" size="20" value="" class="easyui-validatebox" > 小于 <input name="endtime" type="text"  onclick="SelectDate(this,'yyyy-MM-dd hh:mm:ss')" id="endtime" value=""  size="20" class="easyui-validatebox"> 
