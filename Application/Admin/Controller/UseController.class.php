...
class UseController extends BaseController {
    ...
    public function list_page(){
        ...
        if($_SESSION['all'] == 1){
            ...
            $where = 'where `use`.`adminid`='.$_SESSION['adminid'].' and ';
        }
        
        if(!empty($starttime)){
            $starttime = strtotime($starttime);
            $where .=" UNIX_TIMESTAMP(`use`.`createtime`)>='".$starttime."' and";
        }
        if(!empty($endtime)){
            $endtime = strtotime($endtime);
            $where .=" UNIX_TIMESTAMP(`use`.`createtime`)<='".$endtime."' and";
        }
        
        $where = trim($where,' and');
    	$sql = "select `use`.`id`,`use`.`createtime`,`use`.`updatetime` from `use`  ".$where." order by `use`.`".$sort."` ".$order;
