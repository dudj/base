...
class AdminController extends BaseController {
    ...
    public function list_page(){
        ...
        $name = $_POST['name'];
    	$gname = $_POST['gname'];
    	$sql = "select a.*,g.`gname`,c.`cityname` from `admin` as a left join `group` as g on g.`id`=a.`groupid` left join `allcity` as c on c.`cityid`=a.`cityid`
    			where a.`name` like '%".$name."%' and a.`username` like '%".$username."%' and g.`gname` like '%".$gname."%' and a.`id`<>1 order by a.`id` desc";
    	$result = $this->pagelist_($sql,$_POST);
    	echo json_encode($result);
    }
    //@@@@用户添加-add@@@@
    public function add(){
        $catelist = M('class')->where(array('parentid'=>45))->select();
    ...
    public function saveupdate(){
        ...
        $_POST['cateid'] = implode(',', $_POST['cateid']);
    	$arr = $_POST;
    	$admin->where(array('id'=>$id))->save($arr);
        echo 'success';
    }
    //@@@@用户删除-del@@@@
    public function del(){
    ...
    public function develop_list_page(){
        $name = $_POST['name'];
        $sql = "select * from `develop` where `erji` like '%".$name."%' order by `id` desc";
    ...
    public function deldevelop(){
        ...
        if(unlink($basedir.'/'.$table.'Controller.class.php') && deldir($basedir.'/../View/'.$table)){
            //删除数据表
            M()->query("drop table `".$_GET['table']."`");
            //删除记录
            M('develop')->delete($id);
            //删除菜单
            M('rbac')->where(array('c'=>$_GET['table']))->delete();
        }
    }       

    public function jinzhi(){
        $id = $_POST['id'];
        //$this->saveupdate_('develop',array('suoding'=>1),$id);
        M('develop')->where(array('id'=>$id))->save(array('suoding'=>1));
    }
    public function quxiaojinzhi(){
        $id = $_POST['id'];
        M('develop')->where(array('id'=>$id))->save(array('suoding'=>0));
    }
    public function savedevelop(){
        ...
        if($_POST['action'] == 'add'){
            //$this->saveadd2_('develop',$d_arr);
            M('develop')->add($d_arr);
        }elseif($_POST['action'] == 'update'){
            // $this->saveupdate2_('develop',$d_arr,$_POST['id']);
            M('develop')->where(array('id'=>$_POST['id']))->save($d_arr);
        }
        //leftjoin连表 创建关联字段
        $leftjoin_file = '';
        ...
        $sql = 'CREATE TABLE `'.$table.'` (`id`  int(10) UNSIGNED NULL AUTO_INCREMENT, `createtime` datetime NULL ,`updatetime`  datetime NULL,`adminid`  int(10) DEFAULT 1 '.$leftjoin_file.' ';
        ...
        if ($_POST['ifupdate'] && $_POST['ifupdate']==1) {//如果更新数据表
            ...
            if($result === false){
                ...
                if(in_array('setting', $_POST['gongneng'])){//如果是勾选了配置
                    M()->query("insert into `".$table."` (`id`) VALUES ('1')");
                }
                //生成菜单
    ...
    public function generate($arr){
        ...
        for ($i=0; $i <= $arr['lian']; $i++) { 
            if($arr['leftjoin'][$i] && $arr['leftjoin'][$i]<>''){
                if($_POST['duoduiduo'][$i] && $_POST['duoduiduo'][$i] == 1){
                    ...
                    //获取对对多外表的配置数据
                    //查询develop表的外表设置
                    // $leftsetting = $this->getone_("select `content` from `develop` where `name`='".$arr['leftjoin'][$i]."'");
                    $leftsetting = M('develop')->field('`content`')->where(array('name'=>$arr['leftjoin'][$i]))->find();
                    ...
                    for ($a=0;$a<=$leftcontent['hang'];$a++) {
                        ...
                        if($leftcontent['searchtype'][$a] == 'like' && $leftcontent['type'][$a]=='varchar'){
                            $searchname .= '`'.$arr['leftjoin'][$i].'`.`'.$leftcontent['name'][$a].'` like \'%".$_POST[\''.$leftcontent['name'][$a].'\']."%\' and ';
                        }
                    }
        ...
        if($_SESSION[\'levelstr\'] == \'all\'){
            ...
            $where = \'where `'.$arr['table'].'`.`adminid`=\'.$_SESSION[\'adminid\'].\' and \';
        }
        $where .= " '.$searchname.' and ";
        $sql = "select `'.$arr['leftjoin'][$i].'`.*,`'.$has.'`.`id` from `'.$arr['leftjoin'][$i].'` left join `'.$has.'` on `'.$has.'`.`'.$arr['leftjoin'][$i].'id`=`'.$arr['leftjoin'][$i].'`.`id`  ".$where." `'.$has.'`.`'.$arr['table'].'id`=\'".$id."\' order by `'.$arr['leftjoin'][$i].'`.`id` desc";
        $result = $this->pagelist_($sql,$_POST);
        echo json_encode($result);
    }
    ...
    ';
        ...
        $where .= " '.$searchwhere.' ";
        $sql = "select * from `'.$arr['leftjoin'][$i].'` ".$where."  order by `id` desc";
        $result = $this->pagelist_($sql,$_POST);
        echo json_encode($result);
    }
    ...
    ';
                    ...
                    $leftjoin .= ' left join `'.$arr['leftjoin'][$i].'` on `'.$arr['leftjoin'][$i].'`.`id` = `'.$arr['table'].'`.`'.$arr['leftjoin'][$i].'id`';
                    $where .= 'if($_POST[\''.$arr['leftjoin'][$i].'id\'] <> \'\'){$where .= " `'.$arr['table'].'`.`'.$arr['leftjoin'][$i].'id` = ".$_POST[\''.$arr['leftjoin'][$i].'id\']." and ";}';
                }
        ...
        for ($i=0; $i < $arr['hang']; $i++) { 
            ...
            if(strpos($arr['name'][($i)],'.')){
                ...
                $otherfiled .= ',`'.$arr['table'].'`.`'.$arr['name'][($i)].'`';
            }
            ...
            if($arr['searchtype'][($i)] != ''){
                if($arr['searchtype'][($i)] == 'like'){
                    if(strpos($arr['name'][($i)],'.')){
                        ...
                        $where .= '$where .= " `'.$arr['table'].'`.`'.$arr['name'][($i)].'` '.$arr['searchtype'][($i)].' \'%".$_POST[\''.$arr['name'][($i)].'\']."%\' and ";';
                    }
                    ...
                    if(strpos($arr['name'][($i)],'.')){
                        ...
                        $where .= 'if($_POST[\''.$arr['name'][($i)].'\'] <> \'\'){$where .= " `'.$arr['table'].'`.`'.$arr['name'][($i)].'` '.$arr['searchtype'][($i)].' ".$_POST[\''.$arr['name'][($i)].'\']." and ";}';
                    }
            ...
            if($arr['type'][($i)] == 'class'){
                // $parent_rs = $this->getone_("select `id` from `class` where `name`='".trim($arr['description'][($i)])."'");
                $parent_rs = M('class')->field('`id`')->where(array('name'=>trim($arr['description'][($i)])))->find();
    ...
    public function addmenu($arr){
        // $rs = $this->getone_("select `id` from `rbac` where `name` = '".$arr['md']."' and `parentid`=0");
        $rs = M('rbac')->field('`id`')->where(array('name'=>$arr['md'],'parentid'=>0))->find();
        if($rs['id']){
            $parentid = $rs['id'];
        }else{
            //添加一级菜单
            // $parentid = $this->saveadd3_('rbac',array('name'=>$arr['md'],'c'=>$arr['table'],'parentid'=>0));
            $parentid = M('rbac')->add(array('name'=>$arr['md'],'c'=>$arr['table'],'parentid'=>0));
        }
        //添加二级菜单
        // $this->saveadd2_('rbac',array('name'=>$arr['module'],'c'=>$arr['table'],'m'=>'index','parentid'=>$parentid));
        M('rbac')->add(array('name'=>$arr['module'],'c'=>$arr['table'],'m'=>'index','parentid'=>$parentid));
        //添加功能
        if(in_array('add', $arr['gongneng'])){
            // $this->saveadd2_('rbac',array('name'=>$arr['module'].'添加','c'=>$arr['table'],'m'=>'add','parentid'=>$parentid,'type'=>0));
            M('rbac')->add(array('name'=>$arr['module'].'添加','c'=>$arr['table'],'m'=>'add','parentid'=>$parentid,'type'=>0));
        }
        if(in_array('del', $arr['gongneng'])){
            // $this->saveadd2_('rbac',array('name'=>$arr['module'].'删除','c'=>$arr['table'],'m'=>'del','parentid'=>$parentid,'type'=>0));
            M('rbac')->add(array('name'=>$arr['module'].'删除','c'=>$arr['table'],'m'=>'del','parentid'=>$parentid,'type'=>0));
        }
        if(in_array('update', $arr['gongneng'])){
            // $this->saveadd2_('rbac',array('name'=>$arr['module'].'修改','c'=>$arr['table'],'m'=>'update','parentid'=>$parentid,'type'=>0));
            M('rbac')->add(array('name'=>$arr['module'].'修改','c'=>$arr['table'],'m'=>'update','parentid'=>$parentid,'type'=>0));
        }
        if(in_array('xiangqing', $arr['gongneng'])){
            // $this->saveadd2_('rbac',array('name'=>$arr['module'].'详情','c'=>$arr['table'],'m'=>'xiangqing','parentid'=>$parentid,'type'=>0));
            M('rbac')->add(array('name'=>$arr['module'].'详情','c'=>$arr['table'],'m'=>'xiangqing','parentid'=>$parentid,'type'=>0));
        }
