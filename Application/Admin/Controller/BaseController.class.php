...
class BaseController extends Controller {
    ...
    public function __construct(){
        ...
        $c = $url_arr[2];
        $m = ACTION_NAME;
        $data = M('rbac')->field('`id`')->where(array('c'=>$c,'m'=>$m))->find();
    ...
    public function getclass($id=0,$idname='',$selectid='',$limit=''){
        $class = M('class');
        if($selectid!==''){//修改页面里
            //$sql = "select * from `class` where `parentid`=".$id." order by `order` asc ".$limit;
            $where['parentid'] = $id;
            ...
            //$sql = "select * from `class` where `parentid`=".$id." order by `order` asc";
            $where['parentid'] = $id;
            $result = $class->where($where)->order('`order`')->select();
            if($idname !== ''){
    ...
    public function getrbac(){
        if($_SESSION['levelstr'] == 'all'){
            $sql = "select * from `rbac` where `parentid` = 0 order by `order` asc";
        }else{
            $sql = "select * from `rbac` where `parentid` = 0 and `id` in(select `parentid` from `rbac` where `id` in(".$_SESSION['levelstr'].")) order by `order` asc";
        }
        $result = M()->query($sql);
        foreach ($result as $key => $value) {
            if($_SESSION['levelstr'] <> 'all'){
            ...
            $sql2 = "select * from `rbac` where `parentid` = ".$value['id']." and `type`=1 ".$where." order by `order` asc";
    ...
    public function getrbac2(){
        ...
        foreach ($result as $key => $value) {
            ...
            $sql2 = "select * from `rbac` where `parentid` = ".$value['id'].$where." order by `order` asc";
            $result2 = M()->query($sql2);
            $result[$key]['list'] = $result2;
        }
        return $result;
    ...
    public function getclassname($id){
        $data = M('class')->field('`name`')->where(array('id'=>$id))->find();
        return $data['name'];
    }
