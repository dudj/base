...
class RbacController extends BaseController {
    ...
    public function getlist(){
        $sql = "select * from `rbac` where `type`=1 order by `order` asc";
    ...
    public function saveupdate(){
        ...
        unset($_POST['id']);
        $arr = $_POST;
        M('rbac')->where(array('id'=>$id))->save($arr);
    }

    public function del(){
        $id = $_POST['id'];
        M('rbac')->where(array('id'=>$id,'parentid'=>$id))->delete();
    }
...
//         $dir = dir($dirname);
//         //清除表内容
//         $this->query_("delete from `rbac`");
//         $this->query_("TRUNCATE TABLE `rbac`");
...
//                     //插入一级
//                     $rs = $this->getone_("select `id` from `rbac` where `name`='".$one_name_arr[0]."'");
//                     $id = $rs['id'];
//                     if($id){
//                         $this->saveupdate2_('rbac',$arr_one,$id);
//                         $parentid = $id;
//                     }else{
//                         $parentid = $this->saveadd_id('rbac',$arr_one);
//                     }
...
//                             $rs2 = $this->getone_("select `id` from `rbac` where `c` = '".$two_controller."' and `m`= '".$two_method."' and `parentid`=".$parentid);
//                             $id2 = $rs2['id'];
//                             if($id2){
//                                 $this->saveupdate2_('rbac',$arr_two,$id2);
//                             }else{
//                                 $this->saveadd_id('rbac',$arr_two);
//                             }
//                         }
//                     }
...
//                             $rs3 = $this->getone_("select `id` from `rbac` where `c` = '".$two_controller."' and `m`= '".$right_method."' and `parentid`=".$parentid);
//                             $id3 = $rs3['id'];
//                             if($id3){
//                                 $this->saveupdate2_('rbac',$arr_right,$id3);
//                             }else{
//                                 $this->saveadd_id('rbac',$arr_right);
//                             }
//                         }
//                     }
//                 }
