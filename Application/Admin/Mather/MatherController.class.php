...
class ***Table***Controller extends BaseController {
    ...
    public function list_page(){
        ...
        if($_SESSION['all'] == 1){
            ...
            $where = 'where `***table***`.`adminid`='.$_SESSION['adminid'].' and ';
        }
        
        if(!empty($starttime)){
            $starttime = strtotime($starttime);
            $where .=" UNIX_TIMESTAMP(`***table***`.`createtime`)>='".$starttime."' and";
        }
        if(!empty($endtime)){
            $endtime = strtotime($endtime);
            $where .=" UNIX_TIMESTAMP(`***table***`.`createtime`)<='".$endtime."' and";
        }
        ***where***
        $where = trim($where,' and');
    	$sql = "select `***table***`.`id`***otherfiled***,`***table***`.`createtime`,`***table***`.`updatetime` from `***table***` ***leftjoin*** ".$where." order by `***table***`.`".$sort."` ".$order;
